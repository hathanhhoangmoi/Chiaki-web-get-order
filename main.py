...
async def get_delivering_orders(shop_id: str = Query(None)):
    ...
    shops = get_shops_map()
    # Nếu truyền shop_id thì chỉ lấy 1 shop, không thì lấy tất cả
    shop_items = {shop_id: shops[shop_id]}.items() if shop_id and shop_id in shops else shops.items()

    all_orders = []

    async with httpx.AsyncClient(timeout=30) as client:
        for sid, (shop_url, shop_name) in shop_items:
            api_url = (
            ...
            try:
                ...
                col_shipping = fc("mã vận đơn") or fc("vận đơn")
                ...
                for row in ws.iter_rows(min_row=header_row + 1, values_only=True):
                    ...
                    all_orders.append({
                        ...
                        "total":         gv(col_total),
                        "status":        gv(col_status),
                        "shipping_code": gv(col_shipping),
                    })
